...
# Define the path to the firewalld configuration file
config_file="/etc/firewalld/firewall.conf"

# Function to append the ruleset to the config file
append_ruleset() {
    cat >> "$config_file" <<-EOF
table inet firewalld {
	...
	chain mangle_PREROUTING_ZONES {
		iifname "tun0" goto mangle_PRE_public
		iifname "wlo1" goto mangle_PRE_public
	...
	chain nat_PREROUTING_ZONES {
		iifname "tun0" goto nat_PRE_public
		iifname "wlo1" goto nat_PRE_public
		goto nat_PRE_public
	}
	...
	chain nat_POSTROUTING_ZONES {
		oifname "tun0" goto nat_POST_public
		oifname "wlo1" goto nat_POST_public
	...
	chain filter_FORWARD {
		...
		iifname "lo" accept
		ct state invalid drop
		ip6 daddr { ::/96, ::ffff:0.0.0.0/96, 2002::/24, 2002:a00::/24, 2002:7f00::/24, 2002:a9fe::/32, 2002:ac10::/28, 2002:c0a8
		::/32, 2002:e000::/19 } reject with icmpv6 addr-unreachable
    ...
    chain filter_INPUT_ZONES {
        iifname "tun0" goto filter_IN_public
        iifname "wlo1" goto filter_IN_public
    ...
    chain filter_FORWARD_ZONES {
        iifname "tun0" goto filter_FWD_public
        iifname "wlo1" goto filter_FWD_public
...
echo "Config file does not exist. Creating a new one."
touch "$config_file"
fi

# Append the ruleset to the config file
append_ruleset

echo "Firewalld configuration has been updated."
